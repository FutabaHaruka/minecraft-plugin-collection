...
/** One shared cooldown for every crown because the policy is global. */
public final class CooldownService {
    private final Map<UUID, Long> cooldowns = new HashMap<UUID, Long>();

    public long remainingSeconds(Player player, GlobalRule rule) {
        if (rule.getCooldownSeconds() <= 0 || player.hasPermission("crowncontrol.bypass.cooldown")) return 0;
    ...
    public void start(Player player, GlobalRule rule) {
        if (rule.getCooldownSeconds() <= 0 || player.hasPermission("crowncontrol.bypass.cooldown")) return;
