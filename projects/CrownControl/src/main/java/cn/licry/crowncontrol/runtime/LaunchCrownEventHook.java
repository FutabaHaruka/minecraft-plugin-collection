...
/**
 * LaunchClassLoader-side listener for Pixelmon 8.4.x BottleCapEvent.
 *
 * Pixelmon's native InteractionBottleCap posts BottleCapEvent, not the generic
 * ItemInteractionEvent used by mints and several other held-item interactions.
 * Matching the dedicated event and EnumBottleCap value avoids all dependence on
 * translated display names, registry-name spelling or metadata.
 */
public final class LaunchCrownEventHook {
    ...
    /** Kept under the old metric name for command/API compatibility; now counts BottleCapEvent. */
    private final AtomicLong rawItemEvents = new AtomicLong();
    private final AtomicLong nativeCrownEvents = new AtomicLong();
    ...
    public void register(Object plugin, Object service) throws Exception {
        ...
        try { busField = pixelmon.getField("EVENT_BUS"); }
            ...
            busField = pixelmon.getDeclaredField("EVENT_BUS");
        ...
        if (registerMethod == null) {
            throw new NoSuchMethodException(eventBus.getClass().getName() + "#register(Object)");
        }
        ...
        logger.info("CrownControl rc8 BottleCapEvent hook registered through resource-free bytecode bridge.");
    }

    @SubscribeEvent(priority = EventPriority.HIGHEST, receiveCanceled = true)
    public void onPixelmonBottleCap(BottleCapEvent event) {
        rawItemEvents.incrementAndGet();
        if (event == null || event.isCanceled()) return;
        // A broken hot loader may leave this listener referenced after unload.
        // Once unregister() clears the callback state, the listener must be inert.
        ...
        Object player = null;
        try {
            EnumBottleCap bottleCap = event.getBottleCap();
            nativeStack = event.getItemStack();
            String itemDebug = describeNativeItem(nativeStack, bottleCap);

            if (bottleCap != EnumBottleCap.GOLD && bottleCap != EnumBottleCap.SILVER) {
                lastEventDebug = "ignored-unknown-bottle-cap; " + itemDebug;
                return;
            }
            nativeCrownEvents.incrementAndGet();

            player = resolveBukkitPlayer(event);
            if (player == null) {
                cancel(event);
                lastEventDebug = "player-unresolved; " + itemDebug;
                logger.severe("Native Pixelmon BottleCapEvent cancelled because Bukkit Player could not be resolved: "
                        + itemDebug);
                return;
            }
            if (!isPrimaryThread()) {
            ...
            lastEventDebug = "native-crown type=" + bottleCap + ", callback=" + result + "; " + itemDebug;
        } catch (Throwable ex) {
            callbackErrors.incrementAndGet();
            invokeOptional(notifyHandlerError, player);
            cancel(event);
            Throwable root = unwrap(ex);
            lastEventDebug = "callback-error=" + root + "; " + describeNativeItem(nativeStack, safeBottleCap(event));
            if (logger != null) logger.severe("CrownControl BottleCapEvent callback failed: " + root);
        }
    }

    private Object resolveBukkitPlayer(BottleCapEvent event) {
        ...
        try {
            ...
            if (online instanceof Collection) {
                for (Object candidate : (Collection<?>) online) {
    ...
    private static String describeNativeItem(Object nativeStack, Object bottleCap) {
        if (nativeStack == null) return "capType=" + bottleCap + ", stack=null";
        Object item = invokeZeroArg(nativeStack, "getItem", "func_77973_b");
        Object registry = invokeZeroArg(item, "getRegistryName");
        Object translation = invokeZeroArg(item, "getTranslationKey", "getUnlocalizedName", "func_77658_a");
        ...
        return "capType=" + bottleCap
                + ", itemClass=" + (item == null ? "null" : item.getClass().getName())
                + ", registry=" + registry
    ...
    private static Object safePlayer(BottleCapEvent event) {
        try { return event == null ? null : event.getPlayer(); }
        catch (Throwable ignored) { return null; }
    }

    private static Object safeBottleCap(BottleCapEvent event) {
        try { return event == null ? null : event.getBottleCap(); }
        catch (Throwable ignored) { return null; }
    }

    private static void cancel(BottleCapEvent event) {
    ...
    private void invokeOptional(Method method, Object argument) {
        ...
        try { method.invoke(service, argument); }
        catch (Throwable ignored) { }
    }
    ...
    private static Method findOptional(Class<?> type, String name, int count) {
        try { return find(type, name, count); }
        catch (Throwable ignored) { return null; }
    }
    ...
    public void unregister() {
        try {
            if (eventBus != null && unregisterMethod != null) unregisterMethod.invoke(eventBus, this);
        } catch (Throwable ignored) { }
        // Always detach every reference to the Bukkit plugin and its config/service graph.
        // Even if EventBus.unregister fails, a stale listener becomes inert immediately.
        eventBus = null;
    ...
    public long getNativeCrownEvents() { return nativeCrownEvents.get(); }
